...
class LoadImageAnnotationsFromFile(object):
    ...
    def _load_expression_tokenize_beit3(self, results):
        expressions = results["ann"]["expressions"] #특정 박스에 대한 여러 expression
        # choice always the same if 'val'/'test'/'testA'/'testB'
        self.random_ind = np.random.choice(list(range(len(expressions)))) #하나 랜덤 선택
        expression = expressions[self.random_ind] 
        expression = clean_string(expression) #정규식

        
        tokens = self.tokenizer.tokenize(expression) #beit3용 토크나이저로 토큰화
        ...
        # print(self.bos_token_id) #0
        # print(self.eos_token_id) #2
        # print(self.pad_token_id) #1
