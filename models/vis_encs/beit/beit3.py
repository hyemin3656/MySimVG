...
class BEIT3(BEiT3Wrapper):
    ...
    def forward(self, image, question, padding_mask, sentence_token_flag=False, **kwargs):
        outputs = self.beit3(
            textual_tokens=question,
            visual_tokens=image,
            text_padding_position=padding_mask,
            sentence_token_flag=sentence_token_flag
        )
        x = outputs["encoder_out"]
        if sentence_token_flag==True:
            cls_feat, img_feat, text_feat = x[:, 0], x[:, 1 : -(question.shape[-1]+1)], x[:, -(question.shape[-1]+1) :]
        else:
