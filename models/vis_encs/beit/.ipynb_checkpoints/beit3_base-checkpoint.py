...
class Encoder(nn.Module):
    ...
    def forward(
        self,
        src_tokens, #None
        encoder_padding_mask=None, #(bs, 1+num_patches+max_seq_len+1)
        attn_mask=None,
        return_all_hiddens=False,
        token_embeddings=None, #(bs, 1+ num_pat + max_seq_len + 1, emb_dim)
...
class BEiT3(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        #for sentence-level feature----
        self.sentence_token = nn.Parameter(torch.zeros(1, 1, args.encoder_embed_dim))
        ...
        embed_positions = MutliwayEmbedding(
            modules=[
                ...
                PositionalEmbedding(args.max_source_positions, args.encoder_embed_dim), #1024(>max_seq_len이기만 하면 됨), embed_dim 만큼의 학습 가능한 임베딩 행렬 생성 
            ],
            dim=1,
        )
    ...
    def forward(
        ...
        if textual_tokens is None:
            ...
            #sentence-level token 추가
            if sentence_token_flag==True:
                sentence_token = self.sentence_token.expand(
                    textual_tokens.size(0), -1, -1
                ) 
                x2 = torch.cat((x2, sentence_token), dim=1)

            x = torch.cat([x1, x2], dim=1) #(bs, 1+ num_pat + max_seq_len + 1, emb_dim)
            if sentence_token_flag==True:
                sen_token_padding_mask = torch.zeros(text_padding_position.size(0), 1, dtype=text_padding_position.dtype, device=text_padding_position.device)
                text_padding_position = torch.cat([text_padding_position, sen_token_padding_mask], dim=1)

            if text_padding_position is not None:
                encoder_padding_mask = torch.cat( #(bs, 1+ num_pat + max_seq_len + 1)
                    [
        ...
        encoder_out = self.encoder(
            ...
            token_embeddings=x, #(bs, 1+ num_pat + max_seq_len + 1, emb_dim)
            multiway_split_position=multiway_split_position,
